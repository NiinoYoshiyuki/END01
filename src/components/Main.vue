<template>
  <div class="container">
    ...
    <div class="goal_limit_panel" style="margin: 10px;">
      <label>ゴール回数設定：</label>
      <v-text-field
  ...
  setup() {
    ...
    function stopTimer() {
      if (intervalTimer) {
        ...
        currentRunner.value.data.unshift(createRunData(timer.value, null, null, start.value, goal.value))
    ...
    function onGoalTrigger() {
      goalCount.value++

      if (goalCount.value === 1) {
        startTimer()
      }

      if (goalCount.value >= goalLimit.value) {
        stopTimer()
        goalCount.value = 0
      }
    }
